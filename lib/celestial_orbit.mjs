import { Scalar, Vector_3, Angular_Vector_3 } from './vectors.mjs';
    ...
    static filter(value, prefix_system) {
        if (typeof(value) == 'string') {
            ...
            if (match) {
                ...
                if (number && unit) {
                    if (unit.match(/d/i)) {
                        return parseFloat(number) * 86400;
                    }
                } else if (number) {
...
class Field {
	constructor(name, mandatory, representation, type, unit, description, prefix_system) {
		...
		this.prefix_system = (prefix_system === undefined) ? SI_Prefix : prefix_system;
	}
}
...
export class Celestial_Orbit extends Abstract_Object {
	static _fields = [
		...
		new Field('inclination',          true,			'i',   		      Angular_Vector_3,     '°',                      'Inclination', null),
		...
		new Field('mean_anomaly',         true,			'M',   		      'scalar',            '°',                      'Mean Anomaly'),
		new Field('epoch',                true,			't',   		      'string',            null,                      'Epoch (not used yet)'),
		new Field('orbital_period',       true,			'P',   		      Scalar,             Time,                      'Time to complete one orbit')
	]
